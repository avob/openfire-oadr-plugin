...
<jsp:useBean id="pageinfo" scope="request"
	class="org.jivesoftware.admin.AdminPageBean" />
...
<%
	...
	ConnectionManagerImpl connectionManager = ((ConnectionManagerImpl) XMPPServer.getInstance()
			.getConnectionManager());

	// Network interface (if any) is configured for all ports on the server
	AdminConsolePlugin adminConsolePlugin = (AdminConsolePlugin) XMPPServer.getInstance().getPluginManager()
			.getPlugin("admin");
...
.info-table .c2 {
 ...
 %> <%=JiveGlobals.formatDateTime(webManager.getXMPPServer().getServerInfo().getLastStarted())%>
					...
					<tr>
						...
						<td class="c2"><%=AdminConsole.getAppName()%> <%=AdminConsole.getVersionString()%>
						</td>
					</tr>
					<tr>
						<td class="c1"><fmt:message key="index.home" /></td>
						<td class="c2"><%=JiveGlobals.getHomeDirectory()%></td>
					</tr>
					<tr>
						<td class="c1"><fmt:message key="index.server_name" /></td>
						<td class="c2">
							<%
								final IdentityStore identityStore = XMPPServer.getInstance().getCertificateStoreManager()
										.getIdentityStore(ConnectionType.SOCKET_C2S);
							%> <%
 	try {
 %> <%
 	if (!identityStore.containsDomainCertificate()) {
 %> <img src="images/warning-16x16.gif" width="12" height="12"
							border="0" alt="<fmt:message key="index.certificate-warning" />"
							title="<fmt:message key="index.certificate-warning" />">&nbsp;
							<%
								}
							%> <%
 	} catch (Exception e) {
 %> <img src="images/error-16x16.gif" width="12" height="12" border="0"
							alt="<fmt:message key="index.certificate-error" />"
							title="<fmt:message key="index.certificate-error" />">&nbsp;
							<%
								}
							%> <c:out value="${webManager.serverInfo.XMPPDomain}" /> <%
 	try {
 		String whatevs = webManager.getXMPPServer().getServerInfo().getXMPPDomain();
 	} catch (Exception e) {
 %> <img src="images/error-16x16.gif" width="12" height="12" border="0"
							alt="<fmt:message key="index.domain-stringprep-error" />"
							title="<fmt:message key="index.domain-stringprep-error" />">&nbsp;
							<%
								}
							%>
						</td>
				...
				<tbody>
					...
					<tr>
						<td class="c1"><fmt:message key="index.host_name" /></td>
						<td class="c2"><c:out
								value="${webManager.serverInfo.hostname}" /> <%
 	try {
 		String whatevs = webManager.getXMPPServer().getServerInfo().getHostname();
 	} catch (Exception e) {
 %> <img src="images/error-16x16.gif" width="12" height="12" border="0"
							alt="<fmt:message key="index.hostname-stringprep-error" />"
							title="<fmt:message key="index.hostname-stringprep-error" />">&nbsp;
							<%
								}
							%> <%
 	...
 	if (!xmppDomain.equalsIgnoreCase(hostname)) {
 		dnsIssue = true;
 		final List<DNSUtil.WeightedHostAddress> dnsSrvRecords = DNSUtil.srvLookup("xmpp-client", "tcp",
 				xmppDomain);
 	...
 	if (dnsIssue) {
 %> <img src="images/warning-16x16.gif" width="12" height="12"
							border="0"> <fmt:message key="index.dns-warning">
								<fmt:param>
									<a href='dns-check.jsp'>
								</fmt:param>
								<fmt:param>
									</a>
								</fmt:param>
 ...
 %></td>
					...
					<tr>
						...
						<td class="c2"><%=JiveGlobals.getLocale()%> / <%=JiveGlobals.getTimeZone().getDisplayName(JiveGlobals.getLocale())%>
							(<%=(JiveGlobals.getTimeZone().getRawOffset() / 1000 / 60 / 60)%>
							GMT)</td>
					</tr>
					<tr>
								...
								<tr valign="middle">
									<td width="99%" valign="middle">
										<div class="bar">
											<table cellpadding="0" cellspacing="0" border="0"
												width="100%" style="border: 1px #666 solid;">
												<tr>
													<%
														if (percent == 0) {
													%>
													...
													<td width="100%"><img
														src="images/percent-bar-left.gif" width="100%" height="8"
														border="0" alt=""></td>
													...
													<td width="<%=percent%>%"
														...
														src="images/blank.gif" width="1" height="8" border="0"
														alt=""></td>
									...
									<td width="1%" nowrap>
										<div style="padding-left: 6px;" class="c2">
											...
											MB (<%=percentFormat.format(percentUsed)%>%) used
										</div>
									</td>
								</tr>
							</table>
